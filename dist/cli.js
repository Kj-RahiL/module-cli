...
const moduleDir = path_1.default.join(currentDir, "src", "app", "modules", moduleName);
...
//if you wanna add logic here
    const result = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.create({ data });
    return result;
};

const getAll${moduleName}s = async (query: Record<string, any>) => {
    const queryBuilder = new QueryBuilder(prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}, query);
...
const getSingle${moduleName} = async (id: string) => {
    const result = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.findUnique({ where: { id } });
    if(!result){
...
const update${moduleName} = async (id: string, data: any) => {
    const existing${moduleName} = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.findUnique({ where: { id } });
    if (!existing${moduleName}) {
        throw new ApiError(httpStatus.NOT_FOUND, "${moduleName} not found..!!");
    }
    const result = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.update({ where: { id }, data });
    return result;
};
...
const delete${moduleName} = async (id: string) => {
 const existing${moduleName} = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.findUnique({ where: { id } });
    ...
    const result = await prisma.${moduleName.charAt(0).toLowerCase() + moduleName.slice(1)}.delete({ where: { id } });
    return null;
};
...
for (const [fileName, content] of Object.entries(files)) {
    fs_1.default.writeFileSync(path_1.default.join(moduleDir, fileName), content);
}
console.log(`✅ Module "${moduleName}" created successfully in src/modules/${moduleName}`);
